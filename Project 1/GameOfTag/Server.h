...
public:
	...
	void sendGameStart(StartGameData& t_startData, int t_index);
	bool getGameStart(std::shared_ptr<Connection> t_connection, StartGameData& t_startData);

	void sendGameEnd(EndGameData& t_endData);
...
private:
	...
	std::shared_mutex m_mutexConnectionManager; //mutex for managing connections

	SOCKADDR_IN m_addr; //address to bind listening socket
	SOCKET m_listener;
