...
bool Client::processPacketType(const PacketType t_packetType)
{
	...
	case PacketType::Player_set:
	{
		...
		m_game->setStartofGameData(data);
		break;
	}
...
bool Client::getGameStart(StartGameData& t_startData)
{
	int32_t bufferlength;
	if (!getInt32t(bufferlength))
	{
...
bool Client::getGameEnd(EndGameData& t_endData)
{
	int32_t bufferlength;
	if (!getInt32t(bufferlength))
	{
...
void Client::sendGameStart(const StartGameData& t_startData)
{
